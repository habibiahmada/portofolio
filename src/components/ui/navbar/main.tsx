...
export default function Navbar() {
  ...
  const handleSmoothScroll = ({ e, targetId }: SmoothScrollParams): void => {
    e.preventDefault();
    setIsOpen(false);
      ...
      >
        <nav className="container mx-auto px-4 flex items-center justify-between py-4">
          ...
          <div className="text-2xl font-bold">
            <a
              href="#home"
              onClick={(e) => handleSmoothScroll({ e, targetId: "#home" })}
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                onClick={(e) => handleSmoothScroll({ e, targetId: link.href })}
