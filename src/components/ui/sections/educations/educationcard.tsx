...
export function ModernCard({ 
  ...
  return (
    <div
      className={`group relative rounded-xl sm:rounded-2xl transform hover:translate-y-2 w-full transition-transform duration-300 ${
        isDark
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-4 sm:pt-6">
            {exp.skills.map((skill, i) => (
              <span
                key={i}
                className={`px-2.5 py-1 sm:px-3 lg:px-4 sm:py-1.5 lg:py-2 rounded-full text-xs sm:text-sm font-medium hover:scale-105 transition-transform duration-300 ${
                  isDark
