...
const SOCIAL_LINKS = [
  { icon: GithubIcon, label: 'GitHub', href: 'https://github.com/habibiahmada' },
  ...
  return (
    <footer aria-label="Site footer" className="w-full bg-transparent border-t border-black/5 dark:border-white/5 mt-12">
          ...
          <ul className="space-y-2.5" role="list">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors duration-250"
                >
                  {link.label}
                </a>
              </li>
      ...
      <div className="border-t border-black/5 dark:border-white/5 py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row gap-4 items-center justify-between text-[11px] font-mono text-muted-foreground/50 uppercase tracking-wider">
          <p>&copy; {year} Habibi Ahmad Aziz. All rights reserved.</p>
          <p>Next.js · Tailwind v4 · Framer Motion · GSAP</p>
        </div>
      </div>
    </footer>
  )
